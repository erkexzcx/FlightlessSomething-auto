...
yq -c '.jobs[]' "$BENCHMARK_FILE" | while read -r benchmark; do
    ...
    echo "$benchmark" | yq -c '.runs[]' | while read -r run; do
        ...
        sleep 1
        start_time=$(date +%s)
        echo keydown shift+f2 | dotoolc && sleep 0.3 && echo keyup shift+f2 | dotoolc # Start recording
        ...
        echo keydown shift+f2 | dotoolc && sleep 0.3 && echo keyup shift+f2 | dotoolc # Stop recording
        sleep 1
